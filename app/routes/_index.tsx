...
export async function loader() {
  const [allPosts, page] = await Promise.all([
    getAllPublishedPosts(),
    getHomePage(),
  ])
  const latestPosts = allPosts.slice(0, 3)
  return { latestPosts, page }
}

export default function Index() {
  const { latestPosts, page } = useLoaderData<typeof loader>()

  // Parse hero title to add accent to the dot
    ...
    <div className="flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          <div className="space-y-6">
            <h1 className="text-6xl md:text-8xl font-bold tracking-tight">
              {renderHeroTitle(page.heroTitle)}
            </h1>
      ...
      <section className="w-full bg-primary/10 py-16">
        <div className="container px-4 mx-auto max-w-3xl">
          ...
          <p className="mb-6">{page.ctaDescription}</p>
          <Button asChild>
            <Link to="/contact" prefetch="viewport" viewTransition>
              {page.ctaButtonText}
            </Link>
