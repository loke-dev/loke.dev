...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ children }) => <h2 className="mt-6 mb-3 text-2xl font-bold">{children}</h2>,
    h3: ({ children }) => <h3 className="mt-4 mb-2 text-xl font-bold">{children}</h3>,
    ...
    hr: () => <hr className="my-8 border-t border-gray-400" />,
    ...
    blockquote: ({ children }) => (
      <blockquote className="mb-4 border-l-4 border-gray-200 py-2 pl-4 italic">{children}</blockquote>
    ),
    code: ({ children, className }) => {
      const isInlineCode = typeof children === 'string' && !className

      if (isInlineCode) {
        return <code className="rounded bg-gray-100 px-1 py-0.5 font-mono text-sm dark:bg-gray-800">{children}</code>
      }

      return (
        <Suspense fallback={<CodeBlockFallback>{String(children)}</CodeBlockFallback>}>
