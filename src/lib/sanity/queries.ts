...
export const SEARCH_POSTS_QUERY = `*[_type == "post" && !(_id in path("drafts.**")) && (lower(title) match $pattern || lower(description) match $pattern || lower(coalesce(pt::text(body), "")) match $pattern || count(topics[lower(@->title) match $pattern]) > 0 || lower(coalesce(author->name, "")) match $pattern)] | order(date desc) [0...10] { title, "slug": slug.current, description, date }`
