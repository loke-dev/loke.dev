...
export const SEARCH_POSTS_QUERY = `*[_type == "post" && !(_id in path("drafts.**")) && (lower(title) match $pattern || lower(description) match $pattern || lower(coalesce(pt::text(body), "")) match $pattern || count(topics[lower(@->title) match $pattern]) > 0 || lower(coalesce(author->name, "")) match $pattern)] | score(boost(lower(title) match $pattern, 4), boost(lower(description) match $pattern, 2)) | order(_score desc, date desc) [0...10] { title, "slug": slug.current, description, date }`
export const SEARCH_PROJECTS_QUERY = `*[_type == "project" && !(_id in path("drafts.**")) && (lower(title) match $pattern || lower(description) match $pattern || count(coalesce(technologies, [])[lower(@) match $pattern]) > 0)] | score(boost(lower(title) match $pattern, 4), boost(lower(description) match $pattern, 2)) | order(_score desc, order asc, year desc) [0...10] { title, description, url, github }`
